...
for ($i = 0; $i <= 1; $i++){
	...
	//wysyłanie wybranego script_id($skrypt[0]) oraz u_id($skrypt[1])
		...
		if(!socket_connect($sock , 'robozone.no-ip.biz' , 20001))
		{
			$errorcode = socket_last_error();
			$errormsg = socket_strerror($errorcode);
		...
		$message1 = pack("l", $skrypt[0]);
		$message2 = pack("l", $skrypt[1]);
 
		//Send the message1 to the server
		if( !socket_write( $sock , $message1))
		{
			$errorcode = socket_last_error();
			$errormsg = socket_strerror($errorcode);
		...
		echo "Script_id send successfully <br>";	
		
		//Send the message2 to the server
		if(!socket_write($sock, $message2))
		{
			$errorcode = socket_last_error();
			$errormsg = socket_strerror($errorcode);
		...
		echo "u_id send successfully <br>";
		
		if(!socket_write($sock, $value1, strlen($value1)))
		{
			$errorcode = socket_last_error();
			$errormsg = socket_strerror($errorcode);
		...
		echo "filename send successfully <br>";
